...
export class KyselyUserRepository implements UserRepository
{
    ...
    {
        const user = await this.database.selectFrom("users").where("id", "=", id).selectAll().executeTakeFirst();
    ...
    {
        const user = await this.database.selectFrom("users").where("email", "=", email).selectAll().executeTakeFirst();
    ...
    async findByUnique(unique: string): Promise<User | null>
    {
        const user = await this.database
            .selectFrom("users")
            .where((op) => op.or([op("cedula", "=", unique), op("email", "=", unique)]))
            .selectAll()
            .executeTakeFirst();
    ...
    async findManyByFilters(props: FindManyByFiltersParams): Promise<PaginationData<UserDetails[]>>
    {
        ...
        let builder = this.database.selectFrom("users");
        ...
        const count = await builder.select((op) => op.fn.countAll<number>().as("total")).executeTakeFirst();
    ...
    async findManyBySearchQueries(params: QueryDataLimitParams): Promise<UserDetails[]>
    {
        ...
        const people = await this.database
            .selectFrom("users")
            .where((op) => op.or([op("full_name", "like", `%${query}%`), op("cedula", "like", `%${query}%`)]))
            .limit(perPage)
            .selectAll()
            .execute();
    ...
    async delete(person: User): Promise<void>
    {
        ...
        await this.database.deleteFrom("users").where("id", "=", id).executeTakeFirst();
    }
