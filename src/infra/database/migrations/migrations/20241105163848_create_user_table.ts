...
{
    await db.schema.createType("user_genders").asEnum(["FEMENINO", "MASCULINO", "PREFIERO_NO_DECIR"]).execute();

    await db.schema.createType("user_roles").asEnum(["ADMINISTRADOR", "MODERADOR", "ESTUDIANTE", "DOCENTE"]).execute();

    await db.schema.createType("email_status").asEnum(["NOT_VERIFIED", "VERIFIED"]).execute();
